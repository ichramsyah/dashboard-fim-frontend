...
export default function Home() {
  ...
  const StatCard = ({ icon, title, value, color, bgColor }: any) => (
    <div className={`flex-1 px-4 py-3 rounded-lg flex items-center ${bgColor}`}>
      <div className={`p-3 rounded-full mr-1 ${color} bg-white`}>{icon}</div>
      <div>
          ...
          <div className="bg-white px-7 py-6 rounded-lg">
            ...
            <div className="mt-8 px-1 w-full pb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mt-6 gap-4">
                <StatCard icon={<FaShieldAlt size={22} />} title="Total Perubahan" value={todayStats?.total_perubahan_hari_ini ?? 0} color="text-gray-700" bgColor="" />
                <StatCard icon={<FaBell size={22} />} title="Normal" value={todayStats?.detail.normal ?? 0} color="text-blue-500" bgColor="bg-whitee" />
                <StatCard icon={<FaExclamationTriangle size={22} />} title="Mencurigakan" value={todayStats?.detail.mencurigakan ?? 0} color="text-yellow-500" bgColor="bg-whitee" />
                <StatCard icon={<FaExclamationTriangle size={22} />} title="Bahaya" value={todayStats?.detail.bahaya ?? 0} color="text-red-500" bgColor="bg-whitee" />
              </div>
            </div>
          </div>
        )}
