            ...
            {logs.map((log) => (
              <tr key={log.id} className="block md:table-row mb-4 md:mb-0 border md:border-none rounded-lg md:rounded-none">
                ...
                <td data-label="Metode:" className="p-4 flex justify-end md:table-cell text-right md:text-left ">
                  <span
                    className={`px-2 py-1 text-xs font-medium rounded-full ${log.tag.includes('[BAHAYA]') ? 'bg-red-900 text-gray-1' : log.tag.includes('[KEGIATAN MENCURIGAKAN]') ? 'bg-yellow-600 text-gray-1' : 'bg-blue-900 text-gray-1'}`}
                  >
                    {log.metode}
                  </span>
                </td>
